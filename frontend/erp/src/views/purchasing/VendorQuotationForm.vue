<!-- Enhanced Vendor Quotation Form with Dynamic Vendor Loading -->
<template>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <select 
              ...
              v-model="form.rfq_id" 
              required 
              :disabled="isEditMode || loadingVendors"
              @change="loadRfqDetailsAndAvailableVendors"
              class="form-control"
            >
          ...
          <div class="form-group">
            ...
            <select 
              ...
              required
              :disabled="isEditMode || !form.rfq_id || loadingVendors"
              @change="handleVendorChange"
              class="form-control"
            >
              <option value="" disabled>
                {{ getVendorSelectPlaceholder() }}
              </option>
              <option v-for="vendor in availableVendors" :key="vendor.vendor_id" :value="vendor.vendor_id">
                {{ vendor.name }} - {{ vendor.vendor_code }}
                <span v-if="vendor.preferred_currency">({{ vendor.preferred_currency }})</span>
              </option>
            </select>
            <div v-if="errors.vendor_id" class="error-message">{{ errors.vendor_id[0] }}</div>
            
            <!-- Vendor availability info -->
            <div v-if="vendorAvailabilityInfo && form.rfq_id" class="vendor-info">
              <div class="info-row">
                <small class="text-success">
                  <i class="fas fa-check-circle"></i> 
                  {{ vendorAvailabilityInfo.available_vendors_count }} vendors available
                </small>
                <small v-if="vendorAvailabilityInfo.vendors_with_quotations_count > 0" class="text-muted">
                  ({{ vendorAvailabilityInfo.vendors_with_quotations_count }} already submitted)
                </small>
              </div>
            </div>
            
            <!-- No vendors available warning -->
            <div v-if="form.rfq_id && availableVendors.length === 0 && !loadingVendors" class="alert alert-warning">
              <i class="fas fa-exclamation-triangle"></i>
              No vendors available for this RFQ. All vendors may have already submitted their quotations.
            </div>
          </div>
        </div>
      ...
      <div class="form-card currency-card">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <small v-if="selectedVendorPreferredCurrency" class="help-text">
              <i class="fas fa-info-circle"></i>
              Vendor's preferred currency: {{ selectedVendorPreferredCurrency }}
            </small>
          </div>
          ...
          <div class="form-group">
            ...
            <div class="exchange-rate-input">
              ...
              <button 
                ...
                :disabled="!form.currency_code || form.currency_code === baseCurrency || fetchingRate"
...
export default {
  ...
  data() {
    return {
      ...
      rfqOptions: [],
      availableVendors: [], // Changed from vendorOptions to availableVendors
      vendorAvailabilityInfo: null, // Info about vendor availability
      itemOptions: [],
      uomOptions: [],
      currencyOptions: [
      ...
      rfqDetails: null,
      selectedVendorPreferredCurrency: null,
      errors: {}
    };
  },
  computed: {
  ...
  methods: {
    ...
    fetchDropdownOptions() {
      Promise.all([
        axios.get('/request-for-quotations?status=sent'), // Only sent RFQs
        axios.get('/items'),
        axios.get('/uoms')
      ])
      .then(([rfqResponse, itemsResponse, uomResponse]) => {
    ...
    // Load RFQ details and available vendors
    async loadRfqDetailsAndAvailableVendors() {
      if (!this.form.rfq_id) {
        this.availableVendors = [];
        this.vendorAvailabilityInfo = null;
        this.form.vendor_id = '';
        this.form.lines = [];
        return;
      }
      ...
      this.form.vendor_id = ''; // Reset vendor selection
      
      try {
        // Load RFQ details and available vendors simultaneously
        const [rfqResponse, vendorsResponse] = await Promise.all([
          axios.get(`/request-for-quotations/${this.form.rfq_id}`),
          axios.get(`/request-for-quotations/${this.form.rfq_id}/available-vendors`)
        ]);
        ...
        // Handle available vendors
        if (vendorsResponse.data.status === 'success') {
          this.availableVendors = vendorsResponse.data.data.vendors || [];
          this.vendorAvailabilityInfo = {
            available_vendors_count: vendorsResponse.data.data.available_vendors_count,
            vendors_with_quotations_count: vendorsResponse.data.data.vendors_with_quotations_count
          };
          
          if (this.availableVendors.length === 0) {
            this.$toasted.warning('No vendors available for this RFQ. All vendors may have already submitted quotations.');
          }
        } else {
          this.$toasted.error(vendorsResponse.data.message || 'Failed to load available vendors');
          this.availableVendors = [];
          this.vendorAvailabilityInfo = null;
        }
        ...
        console.error('Error loading RFQ details or vendors:', error);
        
        if (error.response && error.response.data && error.response.data.message) {
          this.$toasted.error(error.response.data.message);
        } else {
          this.$toasted.error('Failed to load RFQ details or available vendors');
        }
    ...
    // Get vendor select placeholder text
    getVendorSelectPlaceholder() {
      if (!this.form.rfq_id) {
        return 'Please select RFQ first';
      } else if (this.loadingVendors) {
        return 'Loading available vendors...';
      } else if (this.availableVendors.length === 0) {
        return 'No vendors available for this RFQ';
      } else {
      ...
      axios.get(`/vendor-quotations/${this.localId}`)
        .then(response => {
          if (response.data.status === 'success') {
            ...
            // For edit mode, we don't need to load available vendors
            // since vendor is already selected and cannot be changed
    ...
    handleVendorChange() {
      const selectedVendor = this.availableVendors.find(v => v.vendor_id === this.form.vendor_id);
      if (selectedVendor) {
        this.selectedVendorPreferredCurrency = selectedVendor.preferred_currency;
        
        if (selectedVendor.preferred_currency && selectedVendor.preferred_currency !== this.form.currency_code) {
          // Ask user if they want to use vendor's preferred currency
          if (confirm(`This vendor's preferred currency is ${selectedVendor.preferred_currency}. Do you want to use it?`)) {
            this.form.currency_code = selectedVendor.preferred_currency;
            this.handleCurrencyChange();
          }
        }
      } else {
        this.selectedVendorPreferredCurrency = null;
      }
    },
    ...
    async fetchCurrentExchangeRate() {
      ...
      try {
        const response = await axios.get(`/accounting/currency-rates/current-rate`, {
          params: {
            from_currency: this.form.currency_code,
    ...
    saveQuotation() {
      if (!this.canSubmit) {
        this.$toasted.error('Please complete all required fields');
        return;
      }
      ...
      axios[method](url, formData)
        .then(response => {
          if (response && response.data && response.data.status === 'success') {
            ...
            if (response.data.data && response.data.data.quotation_id) {
